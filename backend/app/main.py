...
CORS. The SPA build is served from ``frontend/dist`` at ``/`` whenever the
directory exists at app-creation time; without a build the mount is
skipped (html=True needs the directory to exist), so API-only deployments
...
class _SPARoute(Route):
    ...
    any path under ``/api/``, letting the router keep walking to the API
    routes; everything else is served from ``frontend/dist`` with
    ``html=True`` (directory requests return ``index.html``).
    """

    def matches(self, scope):
        if scope.get("type") != "http" or scope["path"].startswith("/api/"):
