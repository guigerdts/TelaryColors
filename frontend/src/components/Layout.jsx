...
export default function Layout() {
  ...
  const accountClass = ({ isActive }) =>
    `flex flex-col items-center justify-center gap-0.5 rounded px-2 pb-2 pt-1.5 text-[10px] font-medium leading-none transition focus-visible:ring-2 focus-visible:ring-primary-500/30 active:bg-surface-sunken ${
      isActive ? 'text-primary-500 bg-primary-50' : 'text-text-muted'
    }`

  return (
    <div className="min-h-screen bg-surface-page">
      {/* Header — always sticky; adaptive height via safe-area on mobile. */}
      <header className="sticky top-0 z-20 border-b border-border-default bg-surface-raised">
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4">
          <NavLink
            to="/search"
            className="flex items-center gap-2 font-bold text-text-primary focus-visible:ring-2 focus-visible:ring-primary-500/30"
          >
          ...
          <nav aria-label="Navegación principal" className="hidden items-center gap-1.5 md:flex">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `rounded px-3 py-2 text-sm font-medium transition focus-visible:ring-2 focus-visible:ring-primary-500/30 ${
                    isActive ? 'text-primary-500 bg-primary-50' : 'text-text-secondary hover:bg-surface-sunken'
                  }`
                }
              >
                {link.label}
          ...
          <div className="relative flex items-center">
            <button
              ...
              className="flex items-center gap-1.5 rounded px-2 py-1.5 text-sm font-medium text-text-primary transition hover:bg-surface-sunken focus-visible:ring-2 focus-visible:ring-primary-500/30"
              aria-haspopup="menu"
              aria-expanded={profileOpen}
            >
              <Icon name="user" className="h-5 w-5 text-text-muted" />
              <span className="max-w-[120px] truncate text-text-secondary">
                {user?.full_name || user?.username}
              </span>
              <Icon name="chevron" className="h-3.5 w-3.5 text-text-disabled" />
            </button>

            {profileOpen && (
              <div
                role="menu"
                className="absolute right-0 top-full z-30 mt-2 w-52 overflow-hidden rounded-lg border border-border-default bg-surface-raised py-1 shadow-lg animate-panel-in"
              >
                <div className="border-b border-border-default px-3 py-2">
                  <p className="text-xs font-semibold text-text-muted">Cuenta</p>
                ...
                {user?.role === 'admin' && (
                  <NavLink
                    ...
                    className="block px-3 py-2 text-sm text-text-primary hover:bg-surface-sunken"
                  >
                ...
                <button
                  type="button"
                  onClick={onLogout}
                  className="block w-full px-3 py-2 text-left text-sm text-error-text hover:bg-error-bg"
                >
                  Salir
                </button>
              </div>
      ...
      <nav
        ...
        className="fixed inset-x-0 bottom-0 z-20 border-t border-border-default bg-surface-raised/95 backdrop-blur md:hidden"
      ...
      >
        <div className="flex items-stretch justify-around">
          ...
          <button
            ...
            className="flex min-h-[56px] min-w-0 flex-1 items-center justify-center rounded px-2 pb-2 pt-1.5 text-[10px] font-medium leading-none text-text-muted transition active:bg-surface-sunken focus-visible:ring-2 focus-visible:ring-primary-500/30"
            aria-haspopup="menu"
        ...
        {moreOpen && (
          <>
            ...
            />
            <div className="border-t border-border-default bg-surface-raised animate-panel-in">
              <p className="px-5 pt-3 pb-1 text-xs font-semibold uppercase tracking-wide text-text-muted">
                Más opciones
              </p>
              {visibleSecondary.map((link) => (
                <NavLink
                  ...
                  onClick={() => setMoreOpen(false)}
                    className={({ isActive }) =>
                      `flex items-center gap-3 px-5 py-4 text-sm font-medium text-text-primary transition active:bg-surface-sunken ${
                        isActive ? 'text-primary-500' : ''
                      }`
                    }
                  >
                    <Icon name={link.icon} className="h-5 w-5 text-text-disabled" />
                  {link.label}
                </NavLink>
              ))}
            </div>
